# Tasks

A collection of skills for AI coding agents (Claude Code, etc.), modeled after real software team roles.
Each skill is a `SKILL.md` that teaches the agent how to produce a specific artifact — document, code, diagram, report, or plan.

---

## Naming Convention

```
<type>-<subject>
```

The **type prefix** comes first so skills sort naturally on the filesystem and visually cluster by what they _do_, not what they’re about.

| Prefix       | Produces                                         |
| ------------ | ------------------------------------------------ |
| `audit-`     | Review report with findings and recommendations  |
| `checklist-` | Step-by-step verification list                   |
| `codegen-`   | Source code files                                |
| `design-`    | Design document or spec (contract-first)         |
| `diagram-`   | Visual diagram (C4, DFD, flow, etc.)             |
| `model-`     | Structured model definition                      |
| `patterns-`  | Pattern catalogue with examples                  |
| `planner-`   | Planning document with estimates                 |
| `report-`    | Status or analysis report                        |
| `setup-`     | Configuration files and scaffolding              |
| `strategy-`  | Decision framework and approach                  |
| `template-`  | Reusable blank template                          |
| `tracker-`   | Metrics or progress tracking document            |
| `writer-`    | Authored prose artifact (spec, doc, story, etc.) |

---

## Priority Guide

| Priority | Meaning                                          |
| -------- | ------------------------------------------------ |
| 🔴 P1    | Build first — highest daily leverage, cross-role |
| 🟠 P2    | High value — foundational for a whole domain     |
| 🟡 P3    | Important — core to a specific role              |
| ⚪ P4    | Specialized — niche or lower frequency           |

---

## Skills

Ordered alphabetically by name (matches filesystem order).

### `audit-`

| Done  | Skill              | Priority | Domain       | Primary Role      | Produces                                                                |
| ----- | ------------------ | -------- | ------------ | ----------------- | ----------------------------------------------------------------------- |
| - [x] | `audit-a11y`       | 🟠 P2    | UI/UX        | Frontend / UX     | Accessibility checklist with annotated findings and fix recommendations |
| - [x] | `audit-gap`        | 🟡 P3    | Requirements | System Analyst    | Gap analysis report between current and target state                    |
| - [x] | `audit-secrets`    | 🟠 P2    | Security     | Security Engineer | Secrets exposure report with remediation steps                          |
| - [x] | `audit-security`   | 🟠 P2    | Security     | Security Engineer | OWASP-aligned security review with severity ratings                     |
| - [x] | `audit-test-flaky` | 🟡 P3    | Testing      | AQA               | Root cause report for flaky tests with fix recommendations              |

### `checklist-`

| Done  | Skill                   | Priority | Domain     | Primary Role    | Produces                                          |
| ----- | ----------------------- | -------- | ---------- | --------------- | ------------------------------------------------- |
| - [x] | `checklist-code-review` | 🟠 P2    | Leadership | Team Lead       | Structured PR review checklist by concern area    |
| - [x] | `checklist-release`     | 🟡 P3    | Release    | Release Manager | Go/no-go release checklist with sign-off sections |

### `codegen-`

| Done  | Skill               | Priority | Domain  | Primary Role  | Produces                                                                                                      |
| ----- | ------------------- | -------- | ------- | ------------- | ------------------------------------------------------------------------------------------------------------- |
| - [x] | `codegen-backend`   | 🔴 P1    | Code    | Backend Dev   | Production-ready backend code (Python, Node, Go, Java, Ruby, Rust, C#, PHP, Kotlin, Elixir)                   |
| - [x] | `codegen-frontend`  | 🔴 P1    | Code    | Frontend Dev  | Production-ready frontend code (React, Vue, Angular, Svelte, Next.js, Nuxt, Remix, Astro, SolidJS, SvelteKit) |
| - [x] | `codegen-mobile`    | 🟠 P2    | Code    | Mobile Dev    | Production-ready mobile code (Swift/iOS, Kotlin/Android, React Native, Flutter)                               |
| - [x] | `codegen-test-api`  | 🟠 P2    | Testing | AQA / Backend | API test suite with assertions and fixtures                                                                   |
| - [x] | `codegen-test-e2e`  | 🔴 P1    | Testing | AQA           | End-to-end test scenarios with page objects                                                                   |
| - [x] | `codegen-test-perf` | 🟡 P3    | Testing | AQA / SRE     | Load and stress test scripts with thresholds                                                                  |

### `design-`

| Done  | Skill           | Priority | Domain       | Primary Role       | Produces                                                      |
| ----- | --------------- | -------- | ------------ | ------------------ | ------------------------------------------------------------- |
| - [x] | `design-api`    | 🔴 P1    | Code         | Backend Dev        | Contract-first API spec (OpenAPI / AsyncAPI)                  |
| - [x] | `design-arch`   | 🟠 P2    | Architecture | Solution Architect | System design document with components and tradeoffs          |
| - [x] | `design-css`    | 🟡 P3    | UI/UX        | Frontend Dev       | Design system tokens, component style guide                   |
| - [x] | `design-schema` | 🔴 P1    | Database     | DBA / Backend      | Normalized database schema with relationships and constraints |

### `diagram-`

| Done  | Skill                 | Priority | Domain       | Primary Role       | Produces                                                    |
| ----- | --------------------- | -------- | ------------ | ------------------ | ----------------------------------------------------------- |
| - [x] | `diagram-c4`          | 🟠 P2    | Architecture | Solution Architect | C4 model diagrams (context, container, component, code)     |
| - [x] | `diagram-dfd`         | 🟡 P3    | Requirements | System Analyst     | Data flow diagram (L0 context through L2 process detail)    |
| - [x] | `diagram-integration` | 🟡 P3    | Requirements | System Analyst     | Integration map showing system boundaries and data exchange |
| - [x] | `diagram-ux-flow`     | 🟡 P3    | UI/UX        | UX Designer        | User flows and journey maps                                 |

### `model-`

| Done  | Skill          | Priority | Domain   | Primary Role      | Produces                                           |
| ----- | -------------- | -------- | -------- | ----------------- | -------------------------------------------------- |
| - [x] | `model-dbt`    | 🟡 P3    | Data     | Data Engineer     | dbt model definitions with tests and documentation |
| - [x] | `model-threat` | 🟠 P2    | Security | Security Engineer | STRIDE threat model with mitigations               |

### `patterns-`

| Done  | Skill               | Priority | Domain | Primary Role | Produces                                                                              |
| ----- | ------------------- | -------- | ------ | ------------ | ------------------------------------------------------------------------------------- |
| - [x] | `patterns-auth`     | 🟠 P2    | Code   | Backend Dev  | Auth pattern catalogue (JWT, OAuth2, sessions, API keys) with implementation examples |
| - [x] | `patterns-graphql`  | 🟡 P3    | Code   | Backend Dev  | GraphQL schema, resolver, and N+1 pattern guide                                       |
| - [x] | `patterns-realtime` | 🟡 P3    | Code   | Backend Dev  | WebSocket / SSE / polling pattern guide with tradeoffs                                |

### `planner-`

| Done  | Skill                    | Priority | Domain   | Primary Role | Produces                                           |
| ----- | ------------------------ | -------- | -------- | ------------ | -------------------------------------------------- |
| - [ ] | `planner-capacity-db`    | ⚪ P4    | Database | DBA          | Database capacity plan with growth projections     |
| - [ ] | `planner-capacity-infra` | ⚪ P4    | DevOps   | DevOps / SRE | Infrastructure capacity plan with scaling triggers |
| - [x] | `planner-sprint`         | 🟡 P3    | Agile    | Scrum Master | Sprint plan with goals, capacity, impediment log   |

### `report-`

| Done  | Skill                | Priority | Domain     | Primary Role      | Produces                                                 |
| ----- | -------------------- | -------- | ---------- | ----------------- | -------------------------------------------------------- |
| - [x] | `report-cve`         | 🟡 P3    | Security   | Security Engineer | CVE triage report with severity, impact, and remediation |
| - [x] | `report-db-health`   | 🟡 P3    | Database   | DBA               | Database health report (indexes, bloat, slow queries)    |
| - [ ] | `report-team-health` | ⚪ P4    | Leadership | Team Lead         | Team health report (velocity, satisfaction, blockers)    |

### `setup-`

| Done  | Skill                    | Priority | Domain       | Primary Role      | Produces                                                 |
| ----- | ------------------------ | -------- | ------------ | ----------------- | -------------------------------------------------------- |
| - [ ] | `setup-developer-portal` | ⚪ P4    | Platform     | Platform Engineer | Internal developer portal structure with service catalog |
| - [x] | `setup-eval-harness`     | 🟡 P3    | ML / AI      | ML Engineer       | Model evaluation harness with metrics and baselines      |
| - [x] | `setup-infra`            | 🟠 P2    | DevOps       | DevOps            | Infrastructure-as-code scaffolding (Terraform / Pulumi)  |
| - [x] | `setup-monorepo`         | 🟡 P3    | Platform     | Platform Engineer | Monorepo configuration (Nx, Turborepo, Bazel)            |
| - [x] | `setup-observability`    | 🟠 P2    | DevOps / SRE | DevOps            | Observability stack config (metrics, logs, traces)       |
| - [x] | `setup-pipeline-cicd`    | 🟠 P2    | DevOps       | DevOps            | CI/CD pipeline config (GitHub Actions, GitLab CI, etc.)  |
| - [x] | `setup-pipeline-etl`     | 🟡 P3    | Data         | Data Engineer     | ETL pipeline scaffolding with error handling and logging |
| - [x] | `setup-rag`              | 🟡 P3    | ML / AI      | AI Engineer       | RAG pipeline setup (chunking, embedding, retrieval)      |
| - [x] | `setup-test-framework`   | 🟠 P2    | Testing      | AQA               | Test framework scaffolding with config and conventions   |

### `strategy-`

| Done  | Skill                         | Priority | Domain   | Primary Role        | Produces                                                        |
| ----- | ----------------------------- | -------- | -------- | ------------------- | --------------------------------------------------------------- |
| - [x] | `strategy-api-versioning`     | 🟡 P3    | Code     | Backend Dev         | API versioning strategy with deprecation and migration guide    |
| - [x] | `strategy-backup`             | 🟡 P3    | Database | DBA                 | Backup and recovery strategy with RPO/RTO targets               |
| - [x] | `strategy-dependency-upgrade` | 🟡 P3    | Release  | DevOps / Team Lead  | Dependency upgrade strategy with risk assessment                |
| - [x] | `strategy-feature-flag`       | 🟡 P3    | Code     | Team Lead / Backend | Feature flag strategy with rollout and rollback plan            |
| - [x] | `strategy-test`               | 🟠 P2    | Testing  | AQA / QA            | Test strategy document (scope, levels, tools, coverage targets) |

### `template-`

| Done  | Skill            | Priority | Domain     | Primary Role | Produces                                                        |
| ----- | ---------------- | -------- | ---------- | ------------ | --------------------------------------------------------------- |
| - [x] | `template-pr`    | 🟡 P3    | Leadership | Team Lead    | Pull request template with sections for context, testing, risks |
| - [x] | `template-retro` | 🟡 P3    | Agile      | Scrum Master | Retrospective template (what went well, delta, actions)         |

### `tracker-`

| Done  | Skill              | Priority | Domain | Primary Role | Produces                                       |
| ----- | ------------------ | -------- | ------ | ------------ | ---------------------------------------------- |
| - [ ] | `tracker-velocity` | ⚪ P4    | Agile  | Scrum Master | Sprint velocity report with trend and forecast |

### `writer-`

| Done  | Skill                       | Priority | Domain       | Primary Role          | Produces                                                              |
| ----- | --------------------------- | -------- | ------------ | --------------------- | --------------------------------------------------------------------- |
| - [x] | `writer-adr`                | 🟠 P2    | Architecture | Solution Architect    | Architecture Decision Record (context, decision, consequences)        |
| - [x] | `writer-alert-rules`        | 🟡 P3    | DevOps / SRE | SRE                   | Alert rule definitions with severity and runbook links                |
| - [x] | `writer-api-docs`           | 🟠 P2    | Docs         | Tech Writer / Backend | API reference documentation (endpoints, params, examples)             |
| - [x] | `writer-backlog`            | 🟡 P3    | Agile        | Product Owner         | Groomed backlog with priorities, estimates, and dependencies          |
| - [x] | `writer-changelog`          | 🟡 P3    | Docs         | Tech Writer           | Developer changelog (grouped by type: added, changed, fixed)          |
| - [ ] | `writer-compliance`         | ⚪ P4    | Security     | Security / Legal      | Compliance documentation (GDPR, SOC2, HIPAA)                          |
| - [x] | `writer-epic`               | 🟠 P2    | Agile        | Product Owner         | Epic definition with goal, scope, child stories, DoD                  |
| - [ ] | `writer-lineage`            | ⚪ P4    | Data         | Data Engineer         | Data lineage documentation (sources, transforms, destinations)        |
| - [ ] | `writer-mentorship`         | ⚪ P4    | Leadership   | Team Lead             | Mentorship guide with goals, checkpoints, and resources               |
| - [x] | `writer-migration`          | 🟠 P2    | Database     | DBA                   | Database migration scripts with up/down and validation                |
| - [x] | `writer-ml-experiment`      | 🟡 P3    | ML / AI      | ML Engineer           | ML experiment report with setup, results, and model card              |
| - [x] | `writer-postmortem`         | 🟡 P3    | Leadership   | Team Lead / SRE       | Incident postmortem (timeline, root cause, action items)              |
| - [x] | `writer-prd`                | 🔴 P1    | Requirements | PM / PO               | Product Requirements Document (goals, personas, scope, metrics)       |
| - [x] | `writer-prompt`             | 🟡 P3    | ML / AI      | ML / AI Engineer      | Optimized prompt with system instructions, examples, and eval         |
| - [x] | `writer-readme`             | 🔴 P1    | Docs         | Tech Writer           | Project README (overview, setup, usage, contributing)                 |
| - [x] | `writer-release-notes`      | 🟡 P3    | Docs         | Release Manager       | User-facing release notes (features, fixes, upgrade guide)            |
| - [x] | `writer-runbook`            | 🟡 P3    | Docs         | Tech Writer / SRE     | Operational runbook for routine procedures                            |
| - [x] | `writer-runbook-oncall`     | 🟡 P3    | DevOps / SRE | SRE                   | On-call runbook with alert responses and escalation paths             |
| - [x] | `writer-slo`                | 🟡 P3    | DevOps / SRE | SRE                   | SLO definition with indicators, targets, and error budget             |
| - [x] | `writer-spec-data-contract` | 🟡 P3    | Database     | DBA / Data Engineer   | Data contract spec (schema, ownership, SLA, versioning)               |
| - [x] | `writer-spec-design`        | 🟡 P3    | UI/UX        | UX Designer           | Design spec with component annotations and design tokens              |
| - [x] | `writer-spec-functional`    | 🔴 P1    | Requirements | System Analyst        | Functional requirements spec (actors, flows, business rules)          |
| - [x] | `writer-spec-nfr`           | 🟡 P3    | Requirements | SA / Architect        | Non-functional requirements (performance, scalability, security)      |
| - [x] | `writer-spec-tech`          | 🟠 P2    | Requirements | SA / Architect        | Technical specification (orchestrates functional + nfr + integration) |
| - [x] | `writer-sql`                | 🔴 P1    | Database     | DBA / Backend         | Dialect-aware SQL (Postgres, MySQL, MSSQL, SQLite, Oracle)            |
| - [x] | `writer-sql-analytics`      | 🟡 P3    | Database     | DBA / Data Engineer   | Analytics SQL (Snowflake, BigQuery, ClickHouse, CockroachDB)          |
| - [x] | `writer-stakeholder`        | 🟡 P3    | Agile        | PM / PO               | Stakeholder update (status, risks, decisions needed)                  |
| - [x] | `writer-story-task`         | 🔴 P1    | Agile        | PO / Team Lead        | User stories with acceptance criteria decomposed into dev tasks       |
| - [ ] | `writer-team-agreement`     | ⚪ P4    | Agile        | Scrum Master          | Team working agreement (definition of done, norms, ceremonies)        |
| - [ ] | `writer-tech-radar`         | ⚪ P4    | Architecture | Solution Architect    | Technology radar (adopt, trial, assess, hold)                         |
| - [x] | `writer-use-case`           | 🟡 P3    | Requirements | System Analyst        | Use case document (actors, preconditions, main/alternate flows)       |

---

## Totals

| Priority            | Count  |
| ------------------- | ------ |
| 🔴 P1 — Build first | 10     |
| 🟠 P2 — High value  | 20     |
| 🟡 P3 — Important   | 40     |
| ⚪ P4 — Specialized | 10     |
| **Total**           | **80** |

| Prefix       | Count |
| ------------ | ----- |
| `writer-`    | 31    |
| `setup-`     | 9     |
| `codegen-`   | 6     |
| `audit-`     | 5     |
| `strategy-`  | 5     |
| `design-`    | 4     |
| `diagram-`   | 4     |
| `patterns-`  | 3     |
| `planner-`   | 3     |
| `report-`    | 3     |
| `checklist-` | 2     |
| `model-`     | 2     |
| `template-`  | 2     |
| `tracker-`   | 1     |

---

## Suggested Build Order

Start with P1 — these have the broadest cross-role daily use:

1. `writer-prd` — everything starts with a requirement
2. `writer-story-task` — the most frequently created artifact on any team
3. `writer-spec-functional` — System Analyst core output
4. `design-api` — contract-first, unblocks frontend + backend in parallel
5. `design-schema` — foundational for backend + DBA
6. `writer-sql` — used daily across backend, DBA, and data roles
7. `codegen-backend` — multi-language router, highest daily code volume
8. `codegen-frontend` — multi-framework router, highest daily code volume
9. `codegen-test-e2e` — AQA core, closes the loop on stories
10. `writer-readme` — every repo needs one, immediate visible value
